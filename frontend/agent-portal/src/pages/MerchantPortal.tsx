...
export default function MerchantPortal({ merchantId, merchantName }: MerchantPortalProps) {
  ...
  // Voice guidance per step (blueprint: pre-recorded prompts per language).
  ...
  if (state.step === FLOW_STEPS.SCAN_CARD) {
    content = (
      <StepFrame eyebrow={copy.eyebrows.card} title={copy.scan.title} headingRef={headingRef}>
        ...
        {cardMode === 'camera' && <QrScanner active onValue={handleCardValue} validate={isCardId} invalidMessage={copy.scan.invalid} labels={{ ...copy.scan, useCard: copy.common.useCard }} />}
        {cardMode === 'manual' && <QrScanner active={false} showCamera={false} onValue={handleCardValue} validate={isCardId} invalidMessage={copy.scan.invalid} labels={{ ...copy.scan, useCard: copy.common.useCard, manualLabel: copy.scan.manualOptionTitle, manualHint: copy.scan.manualExample }} />}
    ...
    content = (
      <StepFrame eyebrow={copy.eyebrows.check} title={copy.review.title} headingRef={headingRef}>
        <dl className="merchant-summary">
          ...
          <div><dt>{copy.review.amount}</dt><dd>{formatRupees(state.amountInput)}</dd></div>
    ...
    content = (
      <StepFrame eyebrow={copy.eyebrows.done} title={copy.success.title} headingRef={headingRef}>
        <StatusPanel variant="success" title={copy.success.message}>
          <div className="merchant-result-details"><div><span>{copy.success.amount}</span><strong>{formatRupees(state.submittedAmount)}</strong></div><div><span>{copy.success.transaction}</span><strong>{response.txn_id}</strong></div>{typeof response.new_customer_balance === 'number' && <div><span>{copy.success.balance}</span><strong>{formatRupees(response.new_customer_balance)}</strong></div>}</div>
        </StatusPanel>
        <section className="payment-receipt" aria-label={copy.receipt.title}>
          ...
          <dl>
            ...
            <div><dt>{copy.receipt.date}</dt><dd>{receiptDate.toLocaleString()}</dd></div>
            ...
            <div><dt>{copy.receipt.amount}</dt><dd>{formatRupees(state.submittedAmount)}</dd></div>
            {typeof response.new_customer_balance === 'number' && <div><dt>{copy.receipt.newBalance}</dt><dd>{formatRupees(response.new_customer_balance)}</dd></div>}
          </dl>
